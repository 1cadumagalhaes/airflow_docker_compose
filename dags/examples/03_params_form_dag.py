...
from airflow.sdk import Param, TriggerRule, dag, task


@dag(
    default_args={'owner': 'airflow'},
    dag_id='params_form',
    dag_display_name='03 - Params Form (Trigger UI)',
...
def params_form():
    ...
    def get_names(params=None) -> list[str]:
        if 'names' not in params:
            print('Oops, no names given, was no UI used to trigger?')
            return []
        return params['names']
    ...
    @task(
        task_id='generate_english_greeting',
        task_display_name='Generate English greeting',
    )
    ...
    @task(
        ...
        task_display_name='Generate German greeting',
    )
    ...
    @task(
        ...
        task_display_name='Generate French greeting',
    )
    ...
    print_greetings(english_greetings, german_greetings, french_greetings)
